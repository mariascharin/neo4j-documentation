...
class UnwindTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    section("Introduction", "query-unwind-introduction") {
      ...
      p("""`UNWIND` requires you to specify a new name for the inner values.""".stripMargin)
    }
    section("Unwinding a list", "unwind-unwinding-a-list") {
      ...
      query(
        """UNWIND [1, 2, 3, null] AS x
          |RETURN x, 'val' AS y""".stripMargin, ResultAssertions((r) => {
    ...
    section("Creating a distinct list", "unwind-creating-a-distinct-list") {
      ...
      query(
        """WITH [1, 1, 2, 2] AS coll
          |UNWIND coll AS x
          |WITH DISTINCT x
          |RETURN collect(x) AS setOfVals""".stripMargin, ResultAssertions((r) => {
    ...
    section("Using `UNWIND` with any expression returning a list", "unwind-using-unwind-with-any-expression-returning-a-list") {
      ...
      query(
        """WITH [1, 2] AS a, [3, 4] AS b
          |UNWIND (a + b) AS x
          |RETURN x""".stripMargin, ResultAssertions((r) => {
    ...
    section("Using `UNWIND` with a list of lists", "unwind-using-unwind-with-a-list-of-lists") {
      p(
        """Multiple `UNWIND` clauses can be chained to unwind nested list elements.""".stripMargin)
      query(
        """WITH [[1, 2], [3, 4], 5] AS nested
          |UNWIND nested AS x
          |UNWIND x AS y
          |RETURN y""".stripMargin, ResultAssertions((r) => {
    ...
    section("Using `UNWIND` with an empty list", "unwind-using-unwind-with-an-empty-list") {
      p(
        """Using an empty list with `UNWIND` will produce no rows, irrespective of whether or not any rows existed beforehand, or whether or not other values are being projected.""".stripMargin)
      p(
      ...
      query(
        """UNWIND [] AS empty
          |RETURN empty, 'literal_that_is_not_returned'""".stripMargin, ResultAssertions((r) => {
          r.columnAs[Int]("empty").toList should equal(List())
        })) {
        resultTable()
      }
      p("""To avoid inadvertently using `UNWIND` on an empty list, `CASE` may be used to replace an empty list with a `null`:""".stripMargin)
      p("""[source, cypher]
          |----
          |WITH [] AS list
          |UNWIND
    ...
    section("Using `UNWIND` with an expression that is not a list", "unwind-using-unwind-with-an-expression-that-is-not-a-list") {
      ...
      query(
        """UNWIND null AS x
          |RETURN x, 'some_literal'""".stripMargin, ResultAssertions((r) => {
    ...
    section("Creating nodes from a list parameter", "unwind-creating-nodes-from-a-list-parameter") {
      p(
        """Create a number of nodes and relationships from a parameter-list without using `FOREACH`.""".stripMargin)
      query(
        """UNWIND $events as event
          |MERGE (y:Year {year: event.year})
          |MERGE (y)<-[:IN]-(e:Event {id: event.id})
          |RETURN e.id AS x ORDER BY x""".stripMargin, ResultAssertions((r) => {
          r.toList should equal(List(Map("x" -> 1), Map("x" -> 2)))
        }), ("events", List(Map("year" -> 2014, "id" -> 1), Map("year" -> 2014, "id" -> 2)) )
      ) {
        p(
          """Each value of the original list is unwound and passed through `MERGE` to find or create the nodes and relationships.""".stripMargin)
        resultTable()
      }
    }
  }.build()
